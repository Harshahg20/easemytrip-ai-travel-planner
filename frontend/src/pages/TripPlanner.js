...
export default function TripPlanner() {
  ...
  const loadTripData = useCallback(async () => {
    ...
    try {
      ...
      const fetchedTrip = trips[0];

      if (!fetchedTrip) {
        setError(t("noItineraryFound"));
        setLoading(false);
        return;
      }
      ...
      if (
        fetchedTrip.selected_plan &&
        fetchedTrip.selected_plan.daily_itineraries
      ) {
        setDailyItineraries(fetchedTrip.selected_plan.daily_itineraries);
      } else if (
        fetchedTrip.trip_options &&
        fetchedTrip.trip_options.length > 0
      ) {
        // If no selected plan but options exist, redirect to options page
        navigate(createPageUrl(`TripOptions?trip_id=${tripId}`));
        return;
      } else {
        // No plan selected and no options - this shouldn't happen in normal flow
        setError(t("selectPlanFirst"));
      }
    } catch (err) {
      console.error("Error loading trip:", err);
      setError(t("failedToLoadTrip"));
    } finally {
      setLoading(false);
    }
  }, [t, navigate]);
...
Generated by Tripora - AI Travel Planner
  ...
  const handleSaveCustomization = async (customizedOption) => {
    try {
      await Trip.update(trip.id, {
        selected_plan: customizedOption,
      });
      setTrip({ ...trip, selected_plan: customizedOption });
  ...
  if (showCustomization && trip.selected_plan) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <CustomizationPanel
            option={trip.selected_plan}
            trip={trip}
